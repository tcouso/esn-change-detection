...
def exclude_megadrought_signal(
    fault_detection_dataset_path: Path = paths.data_interim_dir(
        "fault_detection", "fault_detection_dataset.csv"),
    fault_detection_metadata_path: Path = paths.data_interim_dir(
        "fault_detection", "fault_detection_metadata.csv"),
    ...
    fault_detection_df = pd.read_csv(
        fault_detection_dataset_path, index_col=["ID", "IDpix"])
    ...
    fault_detection_metadata_df["change_start"] = pd.to_datetime(
        fault_detection_metadata_df["change_start"])

    megadrought_date = pd.Timestamp(megadrought_threshold)

    pre_megadrought_fault_detection_df = fault_detection_df.loc[:,
                                                                fault_detection_df.columns < megadrought_date]

    labels_to_update = fault_detection_metadata_df[(fault_detection_metadata_df["change_start"] > megadrought_date) & (
        fault_detection_metadata_df["label"] == 1)].index
    ...
    pre_megadrought_fault_detection_metadata_df.loc[labels_to_update,
                                                    "change_type"] = "stable"
    pre_megadrought_fault_detection_metadata_df.loc[labels_to_update, "change_start"] = pd.Timestamp(
        non_change_placeholder_date)
    ...
    create_output_path(out_paths)

    pre_megadrought_fault_detection_df.to_csv(
        pre_megadrought_fault_detection_dataset_path)
    pre_megadrought_fault_detection_metadata_df.to_csv(
        pre_megadrought_fault_detection_metadata_path)
